...
func (r *TodoListPostgres) Create(userId int, list domain.TodoList) (int, error) {
	...
	var id int
	createListQuery := fmt.Sprintf("INSERT INTO %s (title, description) VALUES ($1, $2) RETURNING id", db.TodoListsTable)
	...
	createUsersListQuery := fmt.Sprintf("INSERT INTO %s (user_id, list_id) VALUES ($1, $2)", db.UsersListsTable)
...
func (r *TodoListPostgres) GetById(userId int, listId int) (domain.TodoList, error) {
...
func (r *TodoListPostgres) Update(userId, listId int, list domain.UpdateListInput) error {
